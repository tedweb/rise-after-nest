...
export default function TripExplorer() {
  ...
  return (
    <section className="journey" id="journey">
      <header>
        <p className="sectionLabel">THE JOURNEY SO FAR</p>
        <h2>One trip.<br /><i>Four chapters.</i></h2>
      </header>
      <div className="explorer">
        <div className="map" role="img" aria-label="Stylized map showing stops around Nassau, Bahamas">
          ...
          {stops.map((stop, index) => (
            <button key={stop.title} className={active === index ? "pin active" : "pin"} style={{left: `${stop.x}%`, top: `${stop.y}%`}} onClick={() => setActive(index)} aria-label={`Show ${stop.title}`}>
              <span>{index + 1}</span>
            </button>
          ))}
        ...
        <div className="timeline" aria-label="Trip timeline">
          {stops.map((stop, index) => (
            <button key={stop.title} onClick={() => setActive(index)} className={active === index ? "stop active" : "stop"} aria-pressed={active === index}>
              <span className="date">MAY <b>{stop.day}</b></span>
              <span><small>{stop.label}</small><strong>{stop.title}</strong><em>{stop.note}</em></span>
            </button>
          ))}
        </div>
      </div>
